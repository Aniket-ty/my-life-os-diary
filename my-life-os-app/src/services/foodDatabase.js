export const CURATED_FOOD_DATABASE = [
  // ── PROTEINS ────────────────────────────────────────────────────
  {
...
export function searchLocalFoods(query, maxResults = 8) {
  ...
  for (const item of CURATED_FOOD_DATABASE) {
    ...
    const allTermsMatch = terms.every(
      (term) => lowerName.includes(term) || (item.keywords && item.keywords.some((k) => k.includes(term))),
    );
